...
class VSSBlock(nn.Module):    # Residue State Space Block
    def __init__(
        ...
        self.self_attention = VSSGate(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, para_dict=para_dict, route_dict=route_dict, **kwargs)
...
class MambaRestormer(nn.Module):
    ...
    def __init__(self,
                 ...
                 para_dict={'head_num': 3, 'gfu_t': 0, 'with_proj': True},
                 route_dict_path='./routedicts/route_dict_256_256_4-cr.pth',
                    **kwargs):
        ...
        self.encoder_level1 = BasicLayer(
            ...
            d_state = d_state,
            mlp_ratio=mlp_ratio,
            drop_path=dpr[sum(num_blocks[:0]):sum(num_blocks[:0 + 1])],
            norm_layer=norm_layer,
            downsample=downsample,
            use_checkpoint=use_checkpoint,
            para_dict=para_dict,
            route_dict=route_dict[0]['layer_' + str(0) + '.cxsz_tpinds'].to('cuda')
            )
        ...
        self.encoder_level2 = BasicLayer(
            ...
            d_state = d_state,
            mlp_ratio=mlp_ratio,
            drop_path=dpr[sum(num_blocks[:1]):sum(num_blocks[:1 + 1])],
            norm_layer=norm_layer,
            downsample=downsample,
            use_checkpoint=use_checkpoint,
            para_dict=para_dict,
            route_dict=route_dict[1]['layer_' + str(1) + '.cxsz_tpinds'].to('cuda')
            )
        ...
        self.encoder_level3 = BasicLayer(
            ...
            d_state = d_state,
            mlp_ratio=mlp_ratio,
            drop_path=dpr[sum(num_blocks[:2]):sum(num_blocks[:2 + 1])],
            norm_layer=norm_layer,
            downsample=downsample,
            use_checkpoint=use_checkpoint,
            para_dict=para_dict,
            route_dict=route_dict[2]['layer_' + str(2) + '.cxsz_tpinds'].to('cuda')
        )
        ...
        self.decoder_level2 = BasicLayer(
            ...
            d_state = d_state,
            mlp_ratio=mlp_ratio,
            drop_path=dpr[sum(num_blocks[:1]):sum(num_blocks[:2 + 1])],
            norm_layer=norm_layer,
            downsample=downsample,
            use_checkpoint=use_checkpoint,
            para_dict=para_dict,
